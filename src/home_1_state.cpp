...
Home_1_state::Home_1_state(State_data* state_data): state(state_data)
{
    this->init_key_binds();
    this->init_font();
    this->init_button();
...
void Home_1_state::update(const float &dt)
{
...
void Home_1_state::render(sf::RenderTarget* target)
{
    if (!target) {
        target = this->window;
    }
...
void Home_1_state::update_input(const float &dt)
{
...
void Home_1_state::update_button()
{
...
void Home_1_state::render_button(sf::RenderTarget &target)
{
    for (auto it : this->buttons)
    {
...
void Home_1_state::init_key_binds()
{
    std::ifstream ifs;
