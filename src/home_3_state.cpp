...
Home_3_state::Home_3_state(State_data* state_data): state(state_data)
{
    this->init_key_binds();
    this->init_font();
    this->init_button();
...
void Home_3_state::update(const float &dt)
{
...
void Home_3_state::render(sf::RenderTarget* target)
{
    if (!target) {
        target = this->window;
    }
...
void Home_3_state::update_input(const float &dt)
{
...
void Home_3_state::update_button()
{
    ...
    //     this->states->push(new game_mode_3_state(this->state_data));
    // }
}

void Home_3_state::render_button(sf::RenderTarget &target)
{
    for (auto it : this->buttons)
    {
...
void Home_3_state::init_key_binds()
{
    std::ifstream ifs;
...
void Home_3_state::init_button()
{
    this->buttons["GAMEMODE_3_STATE"] = new gui::button(50.f, 850.f, 200.f, 100.f, &this->font,
